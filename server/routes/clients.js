...
router.put('/:id', asyncHandler(async (req, res) => {
  ...
  // Validate input
  validateClientInput(clientData, true);
...
router.delete('/:id', asyncHandler(async (req, res) => {
  ...
  if (activeAppointments.count > 0) {
    throw new ValidationError('Cannot delete client with active appointments', [
      { field: 'client', message: 'Client has active appointments. Please cancel or complete them first.' }
    ]);
  }
...
router.get('/:id/vehicles', asyncHandler(async (req, res) => {
  ...
  // Check if client exists
  const client = await dbConnection.get('SELECT id FROM clients WHERE id = ?', [id]);
  if (!client) {
    throw new NotFoundError('Client not found');
  }
